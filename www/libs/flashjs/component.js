...
(function(){
	...
	Component.fromPath = function(path, id, async) {
		return Component.wrapPathToElement({path: path, id: id, async:async});
	};
	...
	Component.overrideFromPath = function(scriptElement, path, async) {
		...
		if(typeof async !== "function") {
			...
			Component.fireReadyEvent(component);
		} else {
		...
		while(hasComponents) {
			...
			for(var i = 0; i<x.length; i++) {
				if(x[i].getAttribute('source')) {
					...
					if(x[i].getAttribute('type') == "component-code") {
			...
			req.onreadystatechange  = function() { 
			     if(req.readyState  == 4) {
			    	if(req.status == 200 || req.status == 304) {
			    		...
			    		throw new Error("component not found "+context.path);
				 }
			};
	...
	Component.handleResponse = function(context) {
		...
		var element = Component.wrapToElement(xmlDoc.firstChild, context.id);
	...
	Component.executeScript = function(context, node) {
	...
	Component.wrapToElement = function(node, id, parentDomNode) {
		if(node.nodeName == "script" && node.getAttribute("type") === "component-code") {
			...
			Component.executeScript(parentDomNode, node);
			return null;
		}
		else
		if(node.nodeName == "script" && node.getAttribute("src") !== null) {
			...
			if(node.childNodes.length != 0) // text or cdata
				Component.executeScript(script, node);
		...
		if(node.nodeName == "script" && node.getAttribute("source") !== null) {
			...
			var element = Component.wrapPathToElement({path:node.getAttribute("source"), id:node.getAttribute("id")});
			if(typeof parentDomNode !== 'undefined')
				parentDomNode[node.getAttribute("id")] = element;
			
			if(node.childNodes.length != 0) // text or cdata
				Component.executeScript(element, node);
		...
		if(node.nodeName == "script" && node.getAttribute("source") === null) {
			...
			for(var i = 0; i<node.childNodes.length; i++) {
				...
				if(node.childNodes[i].nodeType == 3) {
					...
					var value = Component.wrapToElement(node.childNodes[i], null, e);
					if(value) {
						e.appendChild(value);
	...
	Component.fireReadyEvent = function(target) {
		...
		for(var i = 0; i<target.childNodes.length; i++)
			Component.fireReadyEvent(target.childNodes[i]);
	};
